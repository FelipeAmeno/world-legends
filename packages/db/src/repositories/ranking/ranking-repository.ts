...
export interface ISeasonRepository {
  ...
  create(input: { code: string; startsAt: Date; endsAt: Date }): Promise<Result<SeasonRow, DbError>>;
}

export interface IRankingRepository {
  findBySeasonAndProfile(seasonId: string, profileId: string): Promise<Result<RankingRow | null, DbError>>;
  findLeaderboard(seasonId: string, limit?: number): Promise<Result<readonly RankingRow[], DbError>>;
  upsert(input: Omit<RankingRow, 'id'>): Promise<Result<RankingRow, DbError>>;
  updateElo(seasonId: string, profileId: string, newElo: number, result: 'win'|'draw'|'loss'): Promise<Result<RankingRow, DbError>>;
}
...
export class SupabaseSeasonRepository implements ISeasonRepository {
  ...
  async findActive(): Promise<Result<SeasonRow | null, DbError>> {
    const { data, error } = await this.db
      .from('seasons').select('*').eq('status', 'active').maybeSingle();
  ...
  async findById(id: string): Promise<Result<SeasonRow | null, DbError>> {
    const { data, error } = await this.db
      .from('seasons').select('*').eq('id', id).maybeSingle();
    if (error) return Err(dbErr(error));
    return Ok(data ? toSeasonRow(data) : null);
  }

  async create(input: { code: string; startsAt: Date; endsAt: Date }): Promise<Result<SeasonRow, DbError>> {
    const { data, error } = await this.db
      .from('seasons')
      .insert({ code: input.code, starts_at: input.startsAt.toISOString(), ends_at: input.endsAt.toISOString() })
      .select('*').single();
...
export class SupabaseRankingRepository implements IRankingRepository {
  ...
  async findBySeasonAndProfile(seasonId: string, profileId: string): Promise<Result<RankingRow | null, DbError>> {
    const { data, error } = await this.db
      .from('rankings')
      .select('*').eq('season_id', seasonId).eq('profile_id', profileId).maybeSingle();
    if (error) return Err(dbErr(error));
    return Ok(data ? toRankingRow(data) : null);
  }

  async findLeaderboard(seasonId: string, limit = 100): Promise<Result<readonly RankingRow[], DbError>> {
    const { data, error } = await this.db
      .from('rankings')
      .select('*')
  ...
  async updateElo(
    seasonId: string, profileId: string,
    newElo: number, result: 'win' | 'draw' | 'loss',
