...
async function main() {
  const { data: profile } = await db.from('profiles').select('id, soft_currency, fragment_balance').eq('username', TEST_USERNAME).single();
  ...
  const { count: ownedBefore } = await db.from('user_cards').select('*', { count: 'exact', head: true }).eq('profile_id', userId);
  ...
  const { data: profAfter } = await db.from('profiles').select('fragment_balance').eq('id', userId).single();
