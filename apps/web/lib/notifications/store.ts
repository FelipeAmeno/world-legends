...
import { KIND_CONFIG, MAX_STORED, NOTIFICATIONS_LS_KEY } from './types';
...
class NotificationStore {
  ...
  private isDuplicate(kind: NotificationKind): boolean {
    ...
    return this.items.some((n) => n.kind === kind && new Date(n.createdAt).getTime() > cutoff);
  }
...
function cfg(kind: NotificationKind) {
...
export function triggerDemoNotifications(): void {
  ...
  setTimeout(
    () =>
      notify.system(
        'Bem-vindo ao World Legends!',
        'Explore e colete as maiores lendas do futebol.',
        '/',
      ),
    900,
  );
