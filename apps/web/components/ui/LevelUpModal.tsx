...
/**
 * components/ui/LevelUpModal.tsx — Sprint 19 (Game Feel & Immersion)
 *
 * Adaptado de components/flow/LevelUpOverlay.tsx (Sprint 3) — a sequência
 * em estágios já existia e já era exatamente o que o briefing pedia
 * (escurece tela → glow dourado → LEVEL UP → novo nível → XP restante →
 * partículas/confetti). O problema não era o visual, era o gatilho: o
 * overlay antigo só lia de um GameContext client-only que nunca disparava
 * de verdade (ver SPRINT_19_REPORT.md). Esta versão é a mesma sequência,
 * controlada por props reais em vez do context morto — qualquer fluxo que
 * detectar um level-up real pode chamá-la.
 */
...
// Partículas determinísticas — sem Math.random() no render
...
export function LevelUpModal({ open, prevLevel, level, onDismiss }: Props) {
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          {/* Particle burst */}
          {(phase === 'burst' || phase === 'title' || phase === 'done') && (
