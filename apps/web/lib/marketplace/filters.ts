...
import type { MarketListing, MarketFilters, MarketSortField } from './types';
...
function sortListings(
  ...
  return [...listings].sort((a, b) => {
    switch (sortBy) {
      ...
      case 'ovr_desc':    return b.cardOvr - a.cardOvr;
      case 'ovr_asc':     return a.cardOvr - b.cardOvr;
...
export function buildMarketFacets(
  listings: readonly MarketListing[],
): MarketFacets {
  ...
  let pMin = Infinity, pMax = 0, oMin = 99, oMax = 0;
  ...
  return {
    rarities: RARITY_ORDER
      .filter(k => rarMap.has(k))
      .map(k => ({ code:k, label:rarMap.get(k)!.label, count:rarMap.get(k)!.count })),
    ...
    countries: [...cntMap.entries()]
      ...
      .sort((a,b) => b.count - a.count),
    eras:      [...eraSet].sort(),
    priceRange:{ min: pMin === Infinity ? 0 : pMin, max: pMax },
...
export function formatPrice(amount: number, unit: string = 'credits'): string {
  if (unit === 'credits') {
