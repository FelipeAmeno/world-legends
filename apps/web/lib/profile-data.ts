...
export function buildTitles(
  ...
  return [
    { id:'first_win', label:'Primeira Vitória',    icon:'⚽', tier:'bronze',
      earned:wins>=1,   desc:'Vença sua primeira partida' },
    { id:'10wins',    label:'Veterano',             icon:'🏆', tier:'bronze',
      earned:wins>=10,  desc:'10 vitórias acumuladas' },
    { id:'25wins',    label:'Experiente',           icon:'🎖️', tier:'silver',
      earned:wins>=25,  desc:'25 vitórias acumuladas' },
    { id:'50wins',    label:'Campeão',              icon:'👑', tier:'gold',
      earned:wins>=50,  desc:'50 vitórias acumuladas' },
    { id:'level5',   label:'Profissional',          icon:'⭐', tier:'bronze',
      earned:level>=5,  desc:'Alcance o nível 5' },
    { id:'level10',  label:'Internacional',         icon:'🌍', tier:'silver',
      earned:level>=10, desc:'Alcance o nível 10' },
    { id:'level15',  label:'Superestrela',          icon:'💫', tier:'gold',
      earned:level>=15, desc:'Alcance o nível 15' },
    { id:'col10',    label:'Colecionador Iniciante',icon:'🃏', tier:'bronze',
      earned:total>=10, desc:'Colete 10 cartas' },
    { id:'col25',    label:'Curador',               icon:'🗄️', tier:'silver',
      earned:total>=25, desc:'Colete 25 cartas' },
    { id:'legendary',label:'Caçador de Lendas',    icon:'✨', tier:'gold',
      earned:hasLegendary, desc:'Obtenha uma carta Lendária' },
    { id:'ultra',    label:'Ultra Raro',            icon:'💎', tier:'platinum',
      earned:hasUltra, desc:'Obtenha uma carta Ultra' },
    { id:'wch',      label:'Herói da Copa',         icon:'🏅', tier:'legendary',
      earned:hasWCH, desc:'Obtenha um World Cup Hero' },
  ];
}
...
export function buildAchievements(
  ...
  const ach = (
    ...
    current:number, target:number, tier:TitleTier,
  ): Achievement => ({
    id, icon, label, desc, tier,
  ...
  return [
    ...
    ach('legendary1','✨','Lenda!',          'Obtenha 1 carta Lendária',
      collection.filter(c=>c.rarityCode==='legendary'||c.rarityCode==='ultra'||c.rarityCode==='world_cup_hero').length, 1, 'gold'),
    ach('nations3', '🌎', '3 Nações',        'Cartas de 3 países',
      new Set(collection.map(c=>c.nationality)).size, 3, 'bronze'),
  ];
}
...
export const SEASONS: Season[] = [
  { id:'s4', label:'T4 – Atual',   position:3, wins:34, losses:12, maxOvr:89, reward:'1.200c',   rewardIcon:'💰', isActive:true },
  { id:'s3', label:'T3 – Passada', position:5, wins:28, losses:16, maxOvr:86, reward:'800c',     rewardIcon:'💰', isActive:false },
  { id:'s2', label:'T2',           position:8, wins:21, losses:21, maxOvr:82, reward:'Classic Pack', rewardIcon:'📦', isActive:false },
  { id:'s1', label:'T1 – Estreia', position:12,wins:14, losses:28, maxOvr:74, reward:'400c',     rewardIcon:'💰', isActive:false },
];
...
export function buildAdvancedStats(collection: CollectionCard[]): AdvancedStats {
  ...
  const avgOvr    = total > 0
    ? Math.round(collection.reduce((s,c) => s+c.overall, 0) / total)
    : 0;
...
export const COUNTRY_FLAGS: Record<string, string> = {
  BR:'🇧🇷', AR:'🇦🇷', FR:'🇫🇷', DE:'🇩🇪', IT:'🇮🇹',
  ES:'🇪🇸', PT:'🇵🇹', NL:'🇳🇱', CM:'🇨🇲', SN:'🇸🇳',
  NG:'🇳🇬', EN:'🏴󠁧󠁢󠁥󠁮󠁧󠁿', US:'🇺🇸', JP:'🇯🇵', MX:'🇲🇽',
  CO:'🇨🇴', CL:'🇨🇱', UY:'🇺🇾', CI:'🇨🇮', GH:'🇬🇭',
  XX:'🌍',
};

export const COUNTRY_NAMES: Record<string, string> = {
  BR:'Brasil', AR:'Argentina', FR:'França', DE:'Alemanha', IT:'Itália',
  ES:'Espanha', PT:'Portugal', NL:'Holanda', CM:'Camarões', SN:'Senegal',
  NG:'Nigéria', EN:'Inglaterra', US:'EUA', JP:'Japão', MX:'México',
  XX:'Outro',
};
