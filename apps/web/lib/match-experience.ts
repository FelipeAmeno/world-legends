...
import { MATCH_OPPONENTS, runMatch } from './match-data';
...
function kindLabel(k: EventKind): string {
  const LABELS: Partial<Record<EventKind, string>> = {
    ...
    injury: 'Lesão',
    sub: 'Substituição',
    save: 'Defesa incrível',
    chance: 'Chance perdida',
    var: 'VAR',
    yellow_home: 'Cartão',
    yellow_away: 'Cartão',
    red_home: 'Expulsão',
...
export function prepareMatchExperience(opponentId: string, seed: number): MatchExperienceData {
