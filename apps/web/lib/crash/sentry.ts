...
export const breadcrumb = {
  ...
  pack: {
    ...
    tapStarted:  (packId:string) =>
      addBreadcrumb('pack', 'Abrindo pack...', { pack_id:packId }),
    exploded:    (packId:string) =>
      addBreadcrumb('pack', 'Pack explodiu!', { pack_id:packId }),
  ...
  match: {
    ...
    completed:  (outcome:string, homeScore:number, awayScore:number) =>
      addBreadcrumb('match', `Partida encerrada: ${outcome}`, { outcome, home:homeScore, away:awayScore }),
  ...
  info: (msg:string, data?:BreadcrumbData) =>
    addBreadcrumb('app', msg, data),
};
