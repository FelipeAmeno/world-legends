...
export function SessionProvider({ children, initialUser=null, initialSession=null }: Props) {
    ...
    const { data: { subscription } } = sb.auth.onAuthStateChange(
      (event, session) => {
        ...
        if (event === 'SIGNED_OUT') {
          ...
          try { localStorage.removeItem('wl-game-state-v1'); } catch {}
        }
      },
    );
  ...
  const signInEmail = useCallback(async (email: string, password: string): Promise<SignInResult> => {
    if (!sb) return { error: null };
    const { error } = await sb.auth.signInWithPassword({ email, password });
    return { error };
  }, [sb]);

  const signInMagicLink = useCallback(async (email: string): Promise<SignInResult> => {
    if (!sb) return { error: null };
