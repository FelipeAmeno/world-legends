...
async function main() {
  const { data: profile } = await db.from('profiles').select('id, soft_currency').eq('username', TEST_USERNAME).single();
  ...
  const rows = result.drawn
    ...
    .map((d) => ({ cardId: d.cardId, userCardId: d.userCardId, acquiredAt: new Date().toISOString() }));
  ...
  console.log(JSON.stringify({
    ...
    sample: enriched.slice(0, 2).map((c) => ({ cardId: c.cardId, displayName: c.displayName, overall: c.overall, rarityCode: c.rarityCode, userCardId: c.userCardId })),
  }, null, 2));
}
