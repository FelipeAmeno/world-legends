...
export class SupabaseMatchRepository implements IMatchRepository {
  ...
  async recordSimulation(
    id: string,
    result: SimulateMatchInput,
  ): Promise<Result<MatchRow, DbError>> {
    ...
    if (result.events.length > 0) {
      ...
      const { error: evErr } = (await (
        ...
        .from('match_events')
        .insert(events)) as { error: { code?: string; message?: string } | null };
  ...
  async getEvents(matchId: string): Promise<Result<readonly MatchEventRow[], DbError>> {
    ...
    const resp = (await this.db
      ...
      .select('*')
      .eq('match_id', matchId)
      .order('minute', { ascending: true })) as unknown as Promise<{
      data: MatchEventRow_DB[] | null;
