...
export interface IUserCardRepository {
  ...
  findByProfileAndCard(profileId: string, cardId: string): Promise<Result<UserCardRow | null, DbError>>;
  ...
  setInjured(id: string, injured: boolean, returnsAtRound?: number): Promise<Result<UserCardRow, DbError>>;
...
export class SupabaseUserCardRepository implements IUserCardRepository {
  ...
  async findByProfileAndCard(profileId: string, cardId: string): Promise<Result<UserCardRow | null, DbError>> {
    const { data, error } = await this.db
      .from('user_cards')
      .select('*')
      .eq('profile_id', profileId)
  ...
  async create(input: CreateUserCardInput): Promise<Result<UserCardRow, DbError>> {
    const { data, error } = await this.db
      ...
      .insert({
        ...
        level: 1, form: 0, is_injured: false,
        suspended_matches: 0, yellow_cards_accum: 0,
      })
      .select('*')
      .single();
  ...
  async updateForm(id: string, form: number): Promise<Result<UserCardRow, DbError>> {
    if (form < -2 || form > 2) {
      return Err(Object.freeze({ code: 'INVALID_FORM', message: `Form deve estar em [-2, +2], recebido: ${form}` }));
    }
    const { data, error } = await this.db
      .from('user_cards').update({ form }).eq('id', id).select('*').single();
    if (error || !data) return Err(dbErr(error));
    return Ok(toUserCardRow(data));
  }

  async setInjured(id: string, injured: boolean, returnsAtRound?: number): Promise<Result<UserCardRow, DbError>> {
    const { data, error } = await this.db
      .from('user_cards')
  ...
  async addYellowCard(id: string): Promise<Result<UserCardRow, DbError>> {
    ...
    if (!found.value) return Err(Object.freeze({ code: 'NOT_FOUND', message: 'UserCard não encontrado' }));
